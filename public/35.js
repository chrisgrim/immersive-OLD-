  ...
  computed: {
    ...
    organizerStyle: function organizerStyle() {
      return function (organizer) {
        return organizer.thumbImagePath ? "background-image:url('/storage/".concat(organizer.thumbImagePath, "')") : "background:".concat(organizer.hexColor);
      };
    }
  },
  methods: {
    ...
    loadEvents: function loadEvents() {
      ...
      axios.get("/create-event/edit/fetch?timestamp=".concat(new Date().getTime())).then(function (response) {
        _this3.organizerEvents = response.data;
      });
    },
